...
Báo cáo này trình bày tổng quan học thuật về bài toán Text-To-Speech (TTS): lịch sử và ba cấp độ phát triển (rule-based, deep-learning, few-shot/zero-shot voice cloning); kiến trúc điển hình
; các kỹ thuật chính để tối ưu hoá ; thách thức nghiên cứu (tốc độ, tài nguyên, đa ngôn ngữ, cảm xúc, đạo đức); và các phương pháp bảo mật như watermark cho giọng tổng hợp để giảm nguy cơ lạm dụng
.

## 1. Giới thiệu

Text-To-Speech (TTS) là công nghệ chuyển đổi văn bản thành giọng nói. Ứng dụng của TTS trải dài từ trợ lý ảo, đọc báo tự động, hỗ trợ người khiếm thị, tạo audiobook, đến các hệ thống tương tác thoại trong công nghiệp và giáo dục. Mục tiêu nghiên cứu bao gồm: chất lượng âm thanh (naturalness), tương đồng giọng (speaker similarity), độ trôi chảy, hiệu suất thời gian thực, và an toàn/đạo đức khi sử dụng voice cloning.
...
- **Rule-based / concatenative / formant** : các hệ thống ghép mẫu hoặc mô tả đặc trưng âm học bằng luật có thể chạy nhẹ nhưng giọng thiếu tự nhiên.

- **Statistical parametric TTS** : dùng mô hình thống kê để dự đoán tham số âm thanh; mượt hơn nhưng vẫn kém tự nhiên so với giọng người.

- **Deep Learning TTS** : sự xuất hiện của mô hình seq2seq và neural vocoder đã nâng chất lượng lên mức rất gần người thật. Tacotron2 báo cáo điểm MOS rất cao trong thí nghiệm của họ.

- **Large generative / few-shot TTS **: các mô hình như VALL-E, Voicebox tiếp cận zero-shot/few-shot cloning, cho phép tổng hợp giọng của người lạ chỉ với vài giây mẫu. Những tiến bộ này mạnh mẽ nhưng đặt ra thách thức đạo đức và bảo mật. ([Microsoft](https://www.microsoft.com/en-us/research/project/vall-e-x/ "VALL-E"))
...
- _Phoneme_ : đơn vị âm nhỏ nhất.

- _Mel-Spectrogram_: biểu diễn năng lượng tần số theo thang Mel.

- _Vocoder_: mô hình chuyển spectrogram thành sóng âm ).

Một số kiến trúc tiêu biểu:

- Tacotron, Tacotron 2. Tacotron 2 (seq2seq acoustic + WaveNet vocoder) — đạt MOS rất cao trong thử nghiệm gốc; FastSpeech / FastSpeech 2 cải thiện tốc độ suy luận; HiFi-GAN và biến thể là vocoder nhanh và chất lượng cao. ([arXiv](https://arxiv.org/abs/1712.05884 "Natural TTS Synthesis by Conditioning WaveNet on Mel Spectrogram Predictions"))
...
### 3.3. Level 3 - Few-shot/Zero-shot TTS

Đây là hướng nghiên cứu hiện đại nhất: chỉ cần 3-10 giây mẫu âm thanh để tái tạo giọng nói của một người.

Dùng mô hình lớn để học biểu diễn giọng đa dạng; chỉ cần vài giây audio mẫu để tổng hợp giọng mới. Ví dụ: VALL-E (Microsoft Research) và Voicebox (Google). ([Microsoft](https://www.microsoft.com/en-us/research/project/vall-e-x "VALL-E"))

**Đặc điểm:**

- Tự nhiên cao
- Giữ được đặc trưng giọng nói
...
**Phù hợp:** dubbing, đạo diễn âm thanh, ứng dụng cần clone giọng nhanh.
...
- **Grapheme-to-Phoneme (G2P)**: chuyển chữ cái => phoneme; giảm phụ thuộc vào hệ chữ viết; rất quan trọng cho đa ngôn ngữ.
...
- **Autoregressive models** : sinh từng frame theo chuỗi; chất lượng cao nhưng chậm. ([arXiv](https://arxiv.org/abs/1712.05884 "Natural TTS Synthesis by Conditioning WaveNet on Mel Spectrogram Predictions"))
...
- **Quantization**: giảm độ chính xác số học để tăng tốc inference.
...
- **Quality vs Speed:** Autoregressive → chất lượng rất cao nhưng latency lớn; FastSpeech + HiFi-GAN → trade-off tốt. ([arXiv](https://arxiv.org/abs/2006.04558 "[2006.04558] FastSpeech 2: Fast and High-Quality End-to-End Text to Speech"))
...
- **Watermarking và detection:** Gắn watermark vào mọi giọng TTS để nhận diện. Các nghiên cứu mới không chỉ phát triển bộ phát mà còn phát triển watermark để đánh dấu âm thanh gốc hoặc phát hiện deepfake. Ví dụ AudioMarkNet là một hướng nghiên cứu watermarking cho audio, nhằm ngăn việc sử dụng giọng thật để fine-tune mô hình TTS độc hại. ([USENIX](https://www.usenix.org/system/files/usenixsecurity25-zong.pdf "Audio Watermarking for Deepfake Speech Detection"))
...
- Tacotron 2 — Shen et al., _Natural TTS Synthesis by Conditioning WaveNet on Mel-Spectrograms_. ([arXiv](https://arxiv.org/abs/1712.05884 "Natural TTS Synthesis by Conditioning WaveNet on Mel Spectrogram Predictions"))
- FastSpeech 2 — Ren et al., _FastSpeech 2: Fast and High-Quality End-to-End Text-to-Speech_. ([arXiv](https://arxiv.org/abs/2006.04558 "[2006.04558] FastSpeech 2: Fast and High-Quality End-to-End Text to Speech"))
